    ...
    for model in model_order:
        try:
            ...
            if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg:
                print(f"⚠️  {model} 配額已滿 (429)，準備切換下一個...")
                continue
            else:
...
def load_used_today() -> list:
    ...
    try:
        with open(USED_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data.get("used_ids", []) if data.get("date") == str(date.today()) else []
...
"""
    ...
    full_prompt = f"{base_prompt}, masterpiece, watercolor texture, ethereal lighting, tranquil, no people, no text"
